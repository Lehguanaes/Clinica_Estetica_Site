...
$cliente = $_POST['cpf_cliente'] ?? ''; // CPF do cliente
$avaliado = $_POST['avaliado'] ?? ''; // Avaliado (pode ser 'Clínica' ou o apelido do profissional)

// Verificação dos dados recebidos
if (!$stars || !$reviewText || !$cliente || !$avaliado) { // aqui usamos $cliente no lugar de $cpf_cliente
    echo "Erro: Preencha todos os campos. Dados recebidos -> Stars: $stars, Comentário: $reviewText, CPF: $cliente, Avaliado: $avaliado";
...
if ($stmt) {
    $stmt->bind_param('siss', $cliente, $stars, $reviewText, $avaliado); // $cliente no lugar de $cpf_cliente
